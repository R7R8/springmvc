...
public class SimpleJdbcTemplateSpitterDao implements
  ...
  private static final String SQL_SELECT_SPITTER_BY_ID = SQL_SELECT_SPITTER
      ...
      private static final String SPITTER_FOR_USERNAME = 
    	      "SELECT * FROM spitter s WHERE s.username = ?";
      private static final String SPITTLES_BY_USERNAME = 
    	      "SELECT * FROM spittle s WHERE s.spitter_id = ?";
  ...
  public List<Spittle> getRecentSpittle() {
	  ...
	  List<Map<String,Object>> maps =jdbcTemplate.queryForList(RECENT_SPITTLES);
	  for(Map<String,Object> map:maps){
		  Spittle spittle = new Spittle();
		  int  id = (Integer) map.get("id");
		  int spitterId = (Integer)map.get("spitter_id");
		  Spitter spitter = getSpitterById(spitterId);
		  String spittleText = (String)map.get("spittleText");
		  Date postedTime = (Date)map.get("postedTime");
		  spittle.setId(new Long(id));
  ...
  private long queryForIdentity() {
    return jdbcTemplate.queryForLong("select auto_increment from information_schema.tables where table_name = 'spitter';");
  }
  ...
  public List<Spittle> getSpittlesForSpitter(
          Spitter spitter) {
	  List<Spittle> spittles = new LinkedList<Spittle>();
	  List<Map<String,Object>> maps =jdbcTemplate.queryForList(SPITTLES_BY_USERNAME,spitter.getId());
	  for(Map<String,Object> map:maps){
		  Spittle spittle = new Spittle();
		  int  id = (Integer) map.get("id");
		  int spitterId = (Integer)map.get("spitter_id");
